...
const createTestAdmins = async () => {
	try {
		...
		console.error(
			"❌ Erreur lors de la création des admins de test:",
			error.message,
		);
	}
};
...
const main = async () => {
	try {
		...
		const createTest = await new Promise((resolve) => {
			rl.question(
				"\nVoulez-vous créer des administrateurs de test? (oui/non): ",
				resolve,
			);
		});

		if (
			createTest.toLowerCase() === "oui" ||
			createTest.toLowerCase() === "o"
		) {
		...
		console.log(
			'💡 Utilisez "node scripts/admin-manager.js" pour gérer les administrateurs',
		);
	} catch (error) {
...
const createAdminIfNeeded = async () => {
	try {
		...
		if (adminCount === 0) {
			...
			console.log(
				`🚀 ${env.toUpperCase()}: Création automatique du premier admin...`,
			);
			await createFirstSuperAdmin();
