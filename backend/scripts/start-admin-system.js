...
`;
  ...
  console.log('⚠️ IMPORTANT: Modifiez les valeurs dans .env avant la production!\n');
}
...
const checkAdmins = async () => {
  try {
    ...
    if (adminCount === 0) {
      ...
      console.log('💡 Exécutez "node scripts/quick-admin-setup.js" pour créer le premier admin\n');
      return false;
    } else {
    ...
    console.log('❌ Erreur lors de la vérification des administrateurs:', error.message);
    return false;
  } finally {
...
const main = async () => {
  try {
    ...
    console.log('🌐 Le serveur sera accessible sur: http://localhost:8000');
    console.log('📚 Documentation API: http://localhost:8000/api-docs');
    console.log('🔧 Interface admin: http://localhost:8000/api/v1/admin');
    console.log('\n💡 Commandes utiles:');
