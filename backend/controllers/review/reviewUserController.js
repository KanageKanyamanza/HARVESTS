...
exports.createReview = catchAsync(async (req, res, next) => {
	...
	if (orderId) {
		...
		if (existingReview) {
			return next(
				new AppError("Vous avez déjà laissé un avis pour cette commande", 400)
			);
		}
	} else {
		...
		if (!completedOrder) {
			...
			if (pendingOrder) {
				return next(
					new AppError(
						`Commande trouvée mais pas encore complétée (statut: ${pendingOrder.status})`,
						403
					)
				);
			}
			...
			const allowReviewsWithoutOrder =
				process.env.ALLOW_REVIEWS_WITHOUT_ORDER === "true";
	...
	if (product) {
		adminNotifications
			.notifyNewReview(review, req.user, product)
			.catch((err) => {
...
exports.voteReview = catchAsync(async (req, res, next) => {
	...
	if (review.reviewer.toString() === req.user.id) {
		return next(
			new AppError("Vous ne pouvez pas voter pour votre propre avis", 400)
		);
	}
...
exports.reportReview = catchAsync(async (req, res, next) => {
	...
	if (reviewer) {
		adminNotifications
			.notifyReviewReported(review, reviewer, reason, description)
			.catch((err) => {
...
// Ajouter une réponse/commentaire à un avis
exports.addReply = catchAsync(async (req, res, next) => {
	...
	if (review.reviewer.toString() !== req.user.id.toString()) {
		await Notification.createNotification({
			...
			type: "review_reply",
			category: "social",
			title: "Nouveau commentaire sur votre avis",
			message: `${req.user.firstName} a commenté votre avis sur un produit`,
			data: {
				reviewId: review._id,
				replyId: review.replies[review.replies.length - 1]._id,
			},
			actions: [
				{
					type: "view",
...
exports.likeReply = catchAsync(async (req, res, next) => {
	...
	const updatedReview = await review.toggleReplyLike(
		req.params.replyId,
		req.user.id
	);
	if (!updatedReview) {
	...
	const reply = updatedReview.replies.id(req.params.replyId);
