...
// @desc    Obtenir les transporteurs disponibles pour une zone de livraison
// @route   GET /api/v1/admin/orders/:id/available-transporters
// @access  Admin
...
// @desc    Assigner un transporteur à une commande
// @route   POST /api/v1/admin/orders/:id/assign-transporter
// @access  Admin
exports.assignTransporterToOrder = catchAsync(async (req, res, next) => {
  ...
  const createDeliveryNotification = async (recipientId, title, message, orderUrl) => {
    return Notification.createNotification({
      ...
      channels: { inApp: { enabled: true }, email: { enabled: true }, push: { enabled: true } }
    });
  };
  ...
  await Notification.createNotification({
    ...
    actions: [{ type: 'view', label: 'Voir la commande', url: transporterOrderUrl }],
    channels: { inApp: { enabled: true }, email: { enabled: true }, push: { enabled: true } }
  });
