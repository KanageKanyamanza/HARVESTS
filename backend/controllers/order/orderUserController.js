...
const { ensureSegmentsForOrder } = require("../../utils/orderSegments");
...
exports.getMyOrders = catchAsync(async (req, res, next) => {
	...
	const query = orderService.buildOrderQuery(
		req.user._id,
		req.user.userType,
		req.query
	);
	...
	for (const order of orders) {
		try {
			...
			console.error(
				`Erreur lors de la création des segments pour la commande ${order._id}:`,
				error.message
			);
		}
	}
	...
	const formattedOrders = orderService.formatOrders(
		orders,
		req.user._id,
		req.user.userType
	);
...
exports.getOrder = catchAsync(async (req, res, next) => {
	...
	const orderData = orderService.formatOrder(
		order,
		req.user._id,
		req.user.userType
	);
...
exports.updateOrderStatus = catchAsync(async (req, res, next) => {
	...
	try {
		const rawItemIds = Array.isArray(itemIds)
			? itemIds
			: itemId
			? [itemId]
			: [];
...
exports.cancelOrder = catchAsync(async (req, res, next) => {
	...
	if (!orderService.checkCancelPermission(order, req.user._id, req.user.role)) {
		return next(
			new AppError("Vous n'avez pas le droit d'annuler cette commande", 403)
		);
	}
	...
	try {
		...
		adminNotifications
			.notifyOrderCancelled(cancelledOrder, req.user, reason)
			.catch((err) => {
