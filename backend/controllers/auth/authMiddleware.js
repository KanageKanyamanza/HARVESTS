 ...
 * Ne renvoie pas d'erreur si non connecté, mais peuple req.user si possible
 */
exports.isLoggedIn = async (req, res, next) => {
	if (
		...
		try {
			...
			// 4) Vérifier si le mot de passe a changé
			if (currentUser.changedPasswordAfter(decoded.iat)) {
				return next();
			}
		...
		if (!req.user.isEmailVerified) {
			...
			return res.status(403).json({
				...
				data: {
					...
					requiresEmailVerification: true,
					suggestion:
						"Vérifiez votre email pour débloquer cette fonctionnalité",
				},
			});
		}
		next();
...
exports.requireApproval = (req, res, next) => {
	if (
		["producer", "transformer", "exporter", "transporter"].includes(
			req.user.userType
		) &&
		!req.user.isApproved
	) {
		return next(
			new AppError(
				"Votre compte doit être approuvé pour accéder à cette fonctionnalité",
				403
			)
		);
	}
