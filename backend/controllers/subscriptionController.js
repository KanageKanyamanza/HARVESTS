...
exports.getSubscription = catchAsync(async (req, res, next) => {
	...
	if (
		...
		return next(
			new AppError("Vous n'avez pas accès à cette souscription", 403)
		);
	}
...
exports.activateFreePlan = catchAsync(async (req, res, next) => {
	...
	if (planId !== "gratuit") {
		return next(
			new AppError("Seul le plan gratuit peut être activé sans paiement", 400)
		);
	}
...
exports.cancelSubscription = catchAsync(async (req, res, next) => {
	...
	subscription.cancellationReason =
		req.body.reason || "Annulé par l'utilisateur";
...
exports.renewSubscription = catchAsync(async (req, res, next) => {
	...
	if (subscription.status !== "active") {
		return next(
			new AppError(
				"Seules les souscriptions actives peuvent être renouvelées",
				400
			)
		);
	}
...
exports.getAllSubscriptions = catchAsync(async (req, res, next) => {
	...
	// Mise à jour automatique des abonnements expirés avant la requête
	const now = new Date();
...
exports.getSubscriptionStats = catchAsync(async (req, res, next) => {
	// Mise à jour automatique des abonnements expirés avant les stats
	const now = new Date();
	await Subscription.updateMany(
		{
			status: "active",
