...
exports.getChatStats = catchAsync(async (req, res, next) => {
	...
	let previousMatchStage = { createdAt: { $lt: new Date(0) } }; // Par défaut : rien (ou impossible)
	...
	const [
		...
		ChatInteraction.distinct("userId", {
			...matchStage,
			userId: { $ne: null },
		}),
		ChatInteraction.aggregate([
	...
	res.status(200).json({
		...
		data: {
			...
			feedbackStats: feedbackStats.reduce((acc, f) => {
				if (f._id === true) acc.helpful = f.count;
				if (f._id === false) acc.not_helpful = f.count;
				return acc;
			}, {}),
			topUnanswered,
			dailyStats,
