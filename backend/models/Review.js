...
reviewSchema.pre('save', function(next) {
  ...
  if (this.isModified('title') || this.isModified('comment') || this.isModified('rating')) {
    if (!this.isNew) {
      this.isEdited = true;
...
reviewSchema.post('save', async function() {
  try {
    ...
    try {
      ...
      console.log('Modèle Producer non disponible pour mise à jour stats:', producerError.message);
    }
  } catch (error) {
...
reviewSchema.statics.getRecentReviews = function(producerId, days = 30, limit = 10) {
