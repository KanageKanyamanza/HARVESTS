...
function addNotificationCreators(notificationSchema) {
	notificationSchema.statics.notifyOrderCreated = async function (order) {
		...
		if (Array.isArray(order.items)) {
			for (const item of order.items) {
				...
				const sellerId =
					getIdString(item.seller) ||
					getIdString(item.productSnapshot?.producer) ||
					getIdString(item.productSnapshot?.transformer) ||
					getIdString(item.productSnapshot?.restaurateur);
				...
				const productStatus =
					item.status || segmentInfo?.status || order.status || "pending";
				...
				async (sellerId) => {
					...
					return this.createNotification({
						...
						message: `Vous avez reçu une nouvelle commande (${
							order.orderNumber
						}) d'un montant de ${sellerAmount || order.total} ${
							order.currency
						}${buyerInfo ? ` de ${buyerInfo.fullName}` : ""}`,
						data: {
							orderId: order._id,
		...
		if (order.delivery?.transporter) {
			const transporterId =
				order.delivery.transporter._id || order.delivery.transporter;
			...
			const transporterNotification = this.createNotification({
				...
				message: `Une nouvelle commande ${
					order.orderNumber
				} vous a été assignée. Frais de livraison : ${
					order.deliveryFee || order.delivery?.deliveryFee || 0
				} ${order.currency}`,
				data: {
					orderId: order._id,
					orderNumber: order.orderNumber,
	...
	notificationSchema.statics.notifyProductApproved = function (
		...
		const notifications = Array.from(ownersMap.keys()).map((ownerId) =>
