...
async function getDeliveryAddresses(userId) {
	...
	// Construire l'objet pour l'adresse principale du profil
	if (user.address || user.city || user.region) {
		const mainAddress = {
			_id: "profile_main_address",
			name: "Adresse Principale (Profil)",
			...
			phone: user.phone,
			isDefault: !deliveryAddresses.some((addr) => addr.isDefault),
			isProfile: true,
		};

		return [mainAddress, ...deliveryAddresses];
	}
...
async function addDeliveryAddress(userId, addressData) {
	const { name, address, city, region, country, postalCode, phone, isDefault } =
		addressData;
