...
async function createSystemNotification(
	adminId,
	{ recipients, type, title, message, channels, scheduledFor, expiresAt }
) {
	...
	const channelStats = await Notification.aggregate([
		{ $match: { createdAt: { $gte: startDate } } },
		{
			$group: {
				...
				emailDelivered: {
					$sum: {
						$cond: [{ $exists: ["$channels.email.deliveredAt", true] }, 1, 0],
					},
				},
				emailOpened: {
					$sum: {
						$cond: [{ $exists: ["$channels.email.openedAt", true] }, 1, 0],
					},
				},
			},
		},
...
async function getPerformanceStats({ period = "30d" }) {
	...
	const clickRate =
		result.total > 0 ? (result.clicked / result.total) * 100 : 0;
