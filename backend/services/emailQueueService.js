...
class EmailQueueService {
  ...
  addToQueue(emailData) {
    ...
    console.log(`📧 Email ajouté à la queue: ${emailData.email}`, { jobId: emailJob.id });
  ...
  async processQueue() {
    ...
    console.log(`🔄 Début du traitement de la queue d'emails (${this.queue.length} emails en attente)`);
    ...
    while (this.queue.length > 0) {
      ...
      try {
        ...
        console.log(`✅ Email envoyé avec succès: ${job.email}`, { jobId: job.id });
        ...
        if (job.retries < this.maxRetries) {
          ...
          console.log(`⏳ Email en retry (${job.retries}/${this.maxRetries}): ${job.email}`, { 
            jobId: job.id, 
            error: err.message 
          });
        } else {
  ...
  cleanupQueue(maxAge = 24 * 60 * 60 * 1000) { // 24 heures par défaut
    ...
    if (removed > 0) {
      console.log(`🧹 Nettoyage de la queue: ${removed} jobs anciens supprimés`);
    }
  }
}
