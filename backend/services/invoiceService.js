 ...
 * @param {String} targetCurrency - Devise cible (ex: EUR)
 * @param {Number} exchangeRate - Taux de change (ex: 655.957)
...
async function generateInvoicePDF(
	...
	try {
		...
		const order = await Order.findById(orderId)
			...
			.populate(
				"seller",
				"farmName companyName firstName lastName email phone address"
			)
			.populate("items.productSnapshot.producer", "farmName firstName lastName")
			.populate(
				"items.productSnapshot.transformer",
				"companyName firstName lastName"
			)
			.populate(
				"items.productSnapshot.restaurateur",
		...
		if (!isAdmin && buyerId !== userId && sellerId !== userId) {
			...
			if (!isItemSeller) {
				throw new AppError(
					"Vous n'avez pas le droit de voir cette facture",
					403
				);
			}
		}
		...
		doc.fontSize(18).font("Helvetica-Bold").text("HARVESTS", companyX, headerY);
		...
		doc
			...
			.text("Tél: +221 78 834 69 69", companyX, headerY + 54);

		// Numéro de facture et date (en haut à droite) - aligné avec le header
		const invoiceNumber =
			order.invoiceNumber ||
			order.orderNumber ||
			`INV-${order._id.toString().substring(0, 8).toUpperCase()}`;
		const invoiceDate = order.createdAt || new Date();

		doc
			.fontSize(9)
			.font("Helvetica")
			.text(`Facture N°: ${invoiceNumber}`, 50, headerY, {
				align: "right",
				width: 500,
			})
			.text(`Date: ${formatDate(invoiceDate)}`, 50, headerY + 14, {
				align: "right",
				width: 500,
			})
			.text(
				`Commande N°: ${order.orderNumber || order._id}`,
				50,
				headerY + 28,
		...
		doc.fontSize(12).font("Helvetica-Bold").text("FACTURÉ À:", 50, yPos);
		...
		if (
			order.seller &&
			order.seller._id.toString() !== buyer?._id?.toString()
		) {
			doc
				.fontSize(12)
				.font("Helvetica-Bold")
			...
			if (seller.email) {
				doc.text(`Email: ${seller.email}`, 400, sellerYPos, {
					align: "right",
					width: 150,
				});
				sellerYPos += 15;
			...
			if (seller.phone) {
				doc.text(`Téléphone: ${seller.phone}`, 400, sellerYPos, {
					align: "right",
					width: 150,
				});
				sellerYPos += 15;
		...
		order.items?.forEach((item, index) => {
			...
			doc.text(
				formatCurrency(unitPrice, targetCurrency, exchangeRate),
				colPriceX,
				yPos,
				{
					width: colTotalX - colPriceX - 10,
			...
			// Total - colonne Total
			doc.text(
				formatCurrency(totalPrice, targetCurrency, exchangeRate),
				colTotalX,
				yPos,
				{
					align: "right",
		...
		if (order.taxes && order.taxes > 0) {
			const label = "TVA:";
			const value = formatCurrency(order.taxes, targetCurrency, exchangeRate);
			doc.text(label, totalsLabelX, yPos, {
				align: "right",
				width: totalsWidth - 130,
			});
		...
		doc
			.fontSize(12)
			.font("Helvetica-Bold")
			.text(totalLabel, totalsLabelX, yPos, {
				align: "right",
				width: totalsWidth - 130,
			});
		...
		if (order.buyerNotes) {
			...
			yPos += 20;
			doc.font("Helvetica").text(order.buyerNotes, 50, yPos, { width: 500 });
		}
		...
		doc
			.fontSize(8)
			.font("Helvetica")
			.text("Merci pour votre confiance !", 50, pageHeight - 60, {
				align: "center",
			})
			.text(
				"Cette facture a été générée automatiquement par Harvests",
				50,
				pageHeight - 45,
				{ align: "center" }
			)
			.text(`Générée le ${formatDate(new Date())}`, 50, pageHeight - 30, {
				align: "center",
			});
 ...
 * Formater une devise
 */
function formatCurrency(amount, currency = "XAF", exchangeRate = 1) {
	if (!amount)
		return `0 ${currency === "XAF" || currency === "FCFA" ? "FCFA" : currency}`;
	...
	const formattedAmount = new Intl.NumberFormat("fr-FR", {
		...
		minimumFractionDigits: currency === "XAF" || currency === "FCFA" ? 0 : 2,
	}).format(convertedAmount);

	// Ajustements spécifiques pour FCFA/XOF si nécessaire, sinon retourner format standard
	if (currency === "FCFA" || currency === "XAF") {
		return formattedAmount.replace("XOF", "FCFA");
	}
