...
async function getMyStats(restaurateurId) {
	...
	const topDishes = await Promise.all(
		Object.entries(dishSales)
			...
			.map(async ([dishId, sales]) => {
				const dish = await Product.findById(dishId).select(
					"name category dishInfo",
				);
				return {
					id: dishId,
	...
	const activeDishes = dishes.filter(
		(d) => d.isActive && d.status === "approved",
	).length;
	const conversionRate =
		dishes.length > 0 ? Math.round((activeDishes / dishes.length) * 100) : 0;
	...
	const restaurateur = await User.findById(restaurateurId);
	const maxWeeklyOrders =
		restaurateur?.subscriptionFeatures?.maxWeeklyOrders || 5;
