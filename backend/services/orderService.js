...
async function processOrderItems(items) {
  ...
  for (const item of items) {
    ...
    if (!sellerIdRaw || !mongoose.Types.ObjectId.isValid(sellerIdRaw)) {
      throw new Error(`Impossible de déterminer un vendeur valide pour le produit ${product._id}`);
    }
...
function checkOrderAccess(order, userId, userRole) {
  ...
  const sellerItemsMatch = order.items?.some(item => {
    ...
    return [itemSeller, producerId, transformerId, restaurateurId].includes(userIdStr);
  });

  const isTransporterOrExporterAssigned = transporterId && transporterId === userIdStr;
  
  return buyerId === userIdStr ||
    sellerId === userIdStr ||
    sellerItemsMatch ||
...
function checkCancelPermission(order, userId, userRole) {
  ...
  return order.buyer.toString() === userId.toString() ||
...
async function createOrder(orderData, user) {
  ...
  const { processedItems, sellerInfoMap } = await processOrderItems(orderData.items);
  ...
  const deliveryFeeDetail = deliveryService.calculateDeliveryFee(processedItems, orderData.deliveryAddress, sellerLocations, deliveryMethod);
  ...
  if (orderData.useLoyaltyPoints && user.userType === 'consumer') {
    ...
    if (consumer && orderData.loyaltyPointsToUse <= consumer.loyaltyProgram.points) {
      discount = await consumer.redeemLoyaltyPoints(orderData.loyaltyPointsToUse);
  ...
  const normalizedPaymentMethod = ['paypal', 'cash'].includes((orderData.paymentMethod || '').toLowerCase())
    ? (orderData.paymentMethod || '').toLowerCase()
    : 'cash';
  const normalizedPaymentProvider = orderData.paymentProvider ||
  ...
  const orderPayload = {
    ...
    delivery: {
      ...
      estimatedDeliveryDate: deliveryService.calculateEstimatedDelivery(deliveryMethod)
    },
...
function buildAllOrdersQuery(queryParams) {
  ...
  if (queryParams.dateTo) {
    queryObj.createdAt = { ...queryObj.createdAt, $lte: new Date(queryParams.dateTo) };
  }
...
async function estimateOrderCosts(items, deliveryAddress, deliveryMethod, user, options = {}) {
  const deliveryService = require('./deliveryService');
  const { processedItems, sellerInfoMap } = await processOrderItems(items);
  const sellerLocations = Array.from(sellerInfoMap.values());
  const subtotal = processedItems.reduce((sum, item) => sum + (item.totalPrice || 0), 0);
  const deliveryFeeDetail = deliveryService.calculateDeliveryFee(processedItems, deliveryAddress, sellerLocations, deliveryMethod);
  const deliveryFee = deliveryFeeDetail.amount;
  const taxes = 0;
  ...
  if (options.useLoyaltyPoints && user.userType === 'consumer') {
    ...
    const consumer = await Consumer.findById(user._id);
    if (consumer) {
      const requestedPoints = Math.max(0, Number(options.loyaltyPointsToUse) || 0);
  ...
  const normalizedPaymentMethod = ['paypal', 'cash'].includes((paymentMethodRaw || '').toLowerCase())
    ...
    : 'cash';
  const normalizedPaymentProvider = (options.paymentProvider ||
    (normalizedPaymentMethod === 'paypal' ? 'paypal' : 'cash-on-delivery'));
