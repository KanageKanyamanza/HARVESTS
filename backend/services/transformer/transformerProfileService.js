...
async function updateTransformerProfile(transformerId, updateData) {
  const allowedFields = [
    'firstName', 'lastName', 'companyName', 'businessName',
  ...
  const transformer = await Transformer.findByIdAndUpdate(transformerId, filteredBody, {
    new: true,
    runValidators: true,
  });
...
async function getCompanyInfo(transformerId) {
  const transformer = await Transformer.findById(transformerId)
    .select('companyName businessName description address contactInfo');
  if (!transformer) {
    throw new Error('Transformateur non trouvé');
  }
  return {
