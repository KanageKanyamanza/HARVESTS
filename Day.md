...
Jour 46 — Page admin : gestion du glossaire de traduction (fait, semence DB à lancer)
...
**Reste à faire** : lancer la semence `node scripts/seedTranslationGlossary.js` (dry-run par défaut, idempotente, n'écrase jamais une entrée existante) puis `--execute` contre Atlas Prod **avec validation explicite** — tant que ce n'est pas fait, le glossaire en prod est vide (la DB est joignable, donc pas de repli JSON). Test de bout en bout de la page contre une vraie base pas encore fait (pas de DB locale, et même le dry-run créerait l'index unique en prod via l'autoIndex de Mongoose).
