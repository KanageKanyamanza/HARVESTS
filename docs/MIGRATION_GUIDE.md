...
Ce guide explique comment migrer progressivement les profils existants vers
l'architecture de services centralisés.

## 🎯 Objectif

Remplacer les services dupliqués par les services centralisés pour améliorer la
...
// Nouveau
const { user } = useAuth();
const { profile, loading, updateProfile, uploadImage, deleteImage } =
  useProfile();
```
...
curl -X GET http://localhost:5001/api/v1/profiles/me
curl -X PATCH http://localhost:5001/api/v1/profiles/me/avatar
```
...
Cette migration garantit une architecture plus robuste et maintenable pour
l'avenir ! 🚀
