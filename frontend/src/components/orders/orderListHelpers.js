...
import { FiClock, FiTruck, FiCheckCircle, FiXCircle, FiPackage } from 'react-icons/fi';
...
export const getStatusConfig = (status) => {
  const configs = {
    ...
    'preparing': { color: 'text-purple-600 bg-purple-100', text: 'En préparation', icon: FiPackage },
    'ready-for-pickup': { color: 'text-orange-600 bg-orange-100', text: 'Prête pour collecte', icon: FiPackage },
    'in-transit': { color: 'text-indigo-600 bg-indigo-100', text: 'En transit', icon: FiTruck },
    'out-for-delivery': { color: 'text-blue-600 bg-blue-100', text: 'En cours de livraison', icon: FiTruck },
    'delivered': { color: 'text-green-600 bg-green-100', text: 'Livrée', icon: FiCheckCircle },
    'completed': { color: 'text-green-600 bg-green-100', text: 'Terminée', icon: FiCheckCircle },
    'cancelled': { color: 'text-red-600 bg-red-100', text: 'Annulée', icon: FiXCircle }
  };
...
export const extractSellerDetails = (order) => {
  ...
  const addSeller = (rawSeller) => {
    ...
    const rawName = sellerObj.farmName || sellerObj.companyName || 
      (sellerObj.firstName && sellerObj.lastName ? `${sellerObj.firstName} ${sellerObj.lastName}` : sellerObj.firstName || sellerObj.lastName || sellerObj.name);
    const displayName = toPlainText(rawName, '');
    if (!displayName) return;

    const key = id || displayName;
    if (!sellersMap.has(key)) {
      sellersMap.set(key, { id: key, name: displayName, email: sellerObj.email || null, phone: sellerObj.phone || null });
    }
  };

  if (order?.segment?.seller) addSeller(order.segment.seller);
  if (Array.isArray(order?.segments)) order.segments.forEach((s) => addSeller(s?.seller));
  if (order?.seller) addSeller(order.seller);
  if (Array.isArray(order?.items)) order.items.forEach((item) => addSeller(item?.seller));
...
export const getClientInfo = (order, userType) => {
  ...
  if (userType === 'producer' || userType === 'transformer') {
    ...
    if (buyer) {
      const name = buyer.firstName && buyer.lastName ? `${buyer.firstName} ${buyer.lastName}` : buyer.name || 'Client';
      return { name, email: buyer.email, phone: buyer.phone };
    }
    return { name: 'Client inconnu' };
  } else {
    if (sellers.length === 1) return sellers[0];
    if (sellers.length > 1) return { name: sellers.map((s) => s.name).join(', ') };
    return null;
  }
};
