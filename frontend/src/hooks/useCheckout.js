...
export const useCheckout = (user, cartItems) => {
			...
			try {
				...
				if (response.data.status === "success") {
					...
					const defaultAddress =
						userData.deliveryAddresses?.find((a) => a.isDefault) ||
						userData.deliveryAddresses?.[0] ||
						userData.address ||
						{};
					const finalCountry = getCountryName(
						userData.country || defaultAddress.country || "SN"
					);
	...
	useEffect(() => {
		if (
			...
			orderData.paymentProvider !== "cash-on-delivery"
		) {
			setOrderData((prev) => ({
				...prev,
				paymentProvider: "cash-on-delivery",
			...
			orderData.paymentProvider !== "paypal"
		) {
	...
	const processCartItems = useCallback(() => {
		const normalized = cartItems.map((item) => {
			if (item.producer?.id)
				return { ...item, originType: item.originType || "product" };
			...
			return {
				...
				producer: { id: fallbackId, name: fallbackName, type: fallbackType },
			};
		});

		return {
			normalized,
			valid: normalized.filter((i) => i.producer?.id),
			invalid: normalized.filter((i) => !i.producer?.id),
		};
	}, [cartItems]);
	...
	const calculateTotals = useCallback(() => {
		const subtotal = cartItems.reduce((sum, item) => {
			...
			if (currencyConfig && currencyConfig.exchangeRate) {
				return sum + amount * currencyConfig.exchangeRate;
			}
			return sum + amount;
		}, 0);
	...
	useEffect(() => {
		...
		orderService
			...
			.then((response) => {
				if (isMounted) setEstimation(response.data?.data || null);
			})
			.catch((error) => {
				if (isMounted) {
					setEstimation(null);
