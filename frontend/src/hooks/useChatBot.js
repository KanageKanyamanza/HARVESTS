...
export const useChatBot = (isAuthenticated, user, cart, clearCartAction) => {
  ...
  useEffect(() => {
    if (guestInfo) sessionStorage.setItem('chat_guest_info', JSON.stringify(guestInfo));
  }, [guestInfo]);
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      ...
      if (isAuthenticated && firstName) {
        addBotMessage(`Bonjour ${firstName} ! 👋 Comment puis-je vous aider aujourd'hui ?`);
        setShowQuickActions(true);
      } else if (guestInfo?.name) {
        addBotMessage(`Re-bonjour ${guestInfo.name.split(' ')[0]} ! 👋 Comment puis-je vous aider ?`);
      } else {
        addBotMessage(faqData.defaultMessages.welcome);
      }
    }
  }, [isOpen, isAuthenticated, guestInfo, messages.length, getUserFirstName, addBotMessage]);
  ...
  return {
    ...
    messagesEndRef, sessionId, getUserFirstName, getUserAvatar,
