...
export const useBlogForm = (id = null) => {
			...
			.then((response) => {
				...
				if (blog) {
					setFormData((prev) => ({
						...
						metaTitle: blog.metaTitle || { fr: "", en: "" },
						metaDescription: blog.metaDescription || { fr: "", en: "" },
	...
	useEffect(() => {
		if (!isEdit) {
			...
			if (savedData) {
				...
				if (savedData.selectedLanguage)
					setSelectedLanguage(savedData.selectedLanguage);
				if (savedData.useAutoTranslation !== undefined)
					setUseAutoTranslation(savedData.useAutoTranslation);
	...
	const addPrerequisite = useCallback(() => {
		if (
			...
			!formData.tutorial.prerequisites.includes(newPrerequisite.trim())
		) {
			setFormData((prev) => ({
				...prev,
				tutorial: {
	...
	const removePrerequisite = useCallback((p) => {
		setFormData((prev) => ({
			...
			tutorial: {
				...prev.tutorial,
				prerequisites: prev.tutorial.prerequisites.filter((x) => x !== p),
			},
		}));
	}, []);

	const handleUploadFeaturedImage = useCallback(
		async (e) => {
