	...
	useEffect(() => {
		const loadProducts = async () => {
			if (user?.userType === "transformer") {
				try {
					setLoading(true);
					// Using getMyProducts as it's the specific endpoint for transformer's own products
		...
		return (
			<div className="min-h-screen relative overflow-hidden p-6 md:p-8 space-y-8">
				<div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden z-0">
					<div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] bg-purple-50/40 rounded-full blur-[120px]"></div>
				</div>
				...
				<div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-fade-in-down">
					<div>
						...
						<p className="text-xs text-gray-500 font-medium max-w-xl">
							Gérez votre inventaire, suivez vos stocks et mettez en valeur vos
				...
				<div className="flex flex-col md:flex-row gap-4 animate-fade-in-up delay-100">
					<div className="relative flex-grow md:max-w-md group">
						...
						<input
							...
							className="block w-full pl-11 pr-4 py-3.5 bg-white/70 backdrop-blur-xl border border-white/60 rounded-2xl text-sm font-bold text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all shadow-sm group-hover:shadow-md"
						/>
					</div>
					...
					<div className="relative min-w-[200px]">
						...
						<select
							...
							className="block w-full pl-10 pr-10 py-3.5 bg-white/70 backdrop-blur-xl border border-white/60 rounded-2xl text-xs font-bold uppercase tracking-wide text-gray-700 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all shadow-sm cursor-pointer appearance-none hover:bg-white"
						>
				...
				{products.length === 0 && !loading ?
					<div className="bg-white/50 backdrop-blur-sm border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center animate-fade-in-up">
						...
						<Link
							...
							className="inline-flex items-center px-5 py-2.5 bg-purple-50 text-purple-700 font-bold rounded-xl text-xs uppercase tracking-wider hover:bg-purple-100 transition-colors"
						>
							Ajouter un produit
						</Link>
					</div>
							...
							return (
								...
								>
										...
										<div className="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
											<div className="flex flex-col gap-2">
												<Link
													...
													className="p-2 bg-white text-gray-700 rounded-full shadow-lg hover:bg-purple-50 hover:text-purple-600 transition-colors"
													title="Modifier"
												>
									...
									<div className="p-5 flex flex-col flex-grow relative">
										...
										<div className="mb-2">
											<span className="text-[10px] font-black text-purple-600 uppercase tracking-[0.2em] bg-purple-50 px-2.5 py-1.5 rounded-lg inline-block">
												{product.category || "Produit"}
											</span>
										</div>

										<h3
											className="text-lg font-[1000] text-gray-900 mb-2 group-hover:text-purple-700 transition-colors line-clamp-1 tracking-tight"
											title={product.name}
										>
											{product.name}
										</h3>
										...
										<div className="pt-5 border-t border-gray-100 flex items-center justify-between mt-auto">
											...
											{product.status === "draft" ?
												<button
													...
													className="text-[10px] bg-purple-100 hover:bg-purple-600 hover:text-white px-5 py-2.5 rounded-xl font-black uppercase tracking-widest transition-all shadow-sm active:scale-95"
												>
													Publier
												</button>
											:	<Link
