...
import { useEditProductTransformer } from "../../../../hooks/useEditProductTransformer";
			...
			<ModularDashboardLayout userType="transformer">
				<div className="max-w-xl mx-auto mt-20 p-10 bg-white/70 backdrop-blur-xl rounded-[2.5rem] border border-white/60 shadow-xl text-center">
					...
					<button
						...
						className="inline-flex items-center px-8 py-3 bg-gray-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-purple-600"
					>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
							...
							<div className="lg:col-span-2 space-y-4 animate-fade-in-up delay-100">
									...
									<div className="space-y-5">
										<div>
											...
											<div className="relative group">
												<FiPackage className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-purple-500 transition-colors" />
												<input
													type="text"
													name="name"
													value={formData.name}
													...
													className={`w-full pl-11 pr-4 py-3 bg-white/50 border rounded-2xl focus:outline-none focus:ring-4 transition-all font-bold text-gray-800 text-sm ${
														...
														:	"border-gray-100 focus:ring-purple-500/5 focus:border-purple-500/50"
													}`}
													placeholder="Ex: Pâte d'arachide"
												/>
											</div>
											{errors.name && (
										...
										<div>
											...
											<div className="relative group">
												<FiAlignLeft className="absolute left-4 top-4 text-gray-400 group-focus-within:text-purple-500 transition-colors" />
												<textarea
													name="description"
								...
								<div className="bg-white/70 backdrop-blur-xl rounded-[1.5rem] border border-white/60 shadow-sm p-4 md:p-6">
									...
									<div className="space-y-6">
										<div className="bg-gray-50/50 rounded-2xl p-4 border border-dashed border-gray-200 hover:border-purple-300 transition-all">
											<ProductImageUpload
											...
											<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
												{productImages.map((image, index) => (
													...
													>
														...
														{image.isPrimary && (
															<div className="absolute top-2 left-2 px-2 py-0.5 bg-purple-500 text-white text-[8px] font-black uppercase tracking-widest rounded-md">
																Principale
															</div>
														)}
													</div>
							...
							<div className="space-y-4 animate-fade-in-up delay-200">
								<div className="bg-white/70 backdrop-blur-xl rounded-[1.5rem] border border-white/60 shadow-sm p-4 md:p-6">
									...
									<div className="space-y-4">
										<div>
											...
											<div className="relative group">
												<FiTag className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-purple-500" />
												<select
													name="category"
										...
										<div className="grid grid-cols-2 gap-3">
											<div>
												...
												<div className="relative group">
													...
													<input
														...
														onChange={handleInputChange}
														className="w-full pl-9 pr-3 py-3 bg-white/50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-purple-500/5 focus:border-purple-500/50 transition-all font-black text-gray-800 text-sm"
														placeholder="0"
													/>
												</div>
											</div>
											<div>
												...
												<select
													...
													onChange={handleInputChange}
													className="w-full px-4 py-3 bg-white/50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-purple-500/5 focus:border-purple-500/50 transition-all font-bold text-gray-600 text-sm appearance-none"
												>
													{CURRENCIES.map((c) => (
										...
										<div className="grid grid-cols-2 gap-3">
											<div>
												...
												<input
													...
													onChange={handleInputChange}
													className="w-full px-4 py-3 bg-white/50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-purple-500/5 focus:border-purple-500/50 transition-all font-black text-gray-800 text-sm"
													placeholder="0"
												/>
											</div>
											<div>
												...
												<select
													...
													value={formData.unit}
													onChange={handleInputChange}
													className="w-full px-4 py-3 bg-white/50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-purple-500/5 focus:border-purple-500/50 transition-all font-bold text-gray-600 text-sm"
												>
													{UNITS.map((u) => (
								...
								{/* Action Box */}
								<div className="bg-purple-600 rounded-[2rem] p-6 text-white shadow-xl shadow-purple-200/50 relative overflow-hidden group">
									<div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform duration-700">
										<FiSave className="w-24 h-24" />
									</div>
									<div className="relative z-10 space-y-4">
										<p className="text-[10px] font-black text-purple-200 uppercase tracking-[0.2em]">
											Validation Finale
										</p>
										...
										<div className="pt-2 flex flex-col gap-3">
											<button
												type="submit"
												disabled={saving}
												className="w-full inline-flex items-center justify-center px-6 py-4 bg-white text-purple-600 font-black text-[10px] uppercase tracking-widest rounded-xl hover:shadow-xl hover:-translate-y-1 transition-all disabled:opacity-50"
											>
												{saving ?
													<div className="flex items-center gap-2">
														<div className="w-3 h-3 border-2 border-purple-600/20 border-t-purple-600 rounded-full animate-spin"></div>
											...
											<button
												type="button"
												onClick={() => navigate("/transformer/products")}
												className="w-full inline-flex items-center justify-center px-6 py-3 bg-purple-500/20 border border-purple-400/30 text-white font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-purple-500/40 transition-all"
											>
												Annuler
											</button>
										</div>
