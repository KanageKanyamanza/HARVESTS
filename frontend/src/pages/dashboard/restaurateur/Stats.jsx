			...
			if (user?.userType === "restaurateur") {
				try {
					...
					const formattedStats = {
						...rawStats,
						topProducts:
							Array.isArray(rawStats?.topProducts) ?
					...
					const receivedOrders = ordersData.filter((order) => {
						...
						if (
							Array.isArray(order.segment?.items) &&
							order.segment.items.length > 0
						)
							return true;
						if (
							Array.isArray(order.segments) &&
					...
					<div className="space-y-2 max-h-64 overflow-y-auto">
						{(
							...
							salesAnalytics.monthlySales.slice(0, 6).map((sale, index) => (
								<div
									key={index}
									className="flex items-center justify-between p-3 bg-harvests-light rounded-lg"
								>
									<div className="flex-1">
										...
										<p className="text-xs text-gray-500">
											{sale.orders} commande{sale.orders > 1 ? "s" : ""}
										</p>
									</div>
									<div className="text-right">
										...
										<p className="text-xs text-green-600 font-medium">
											{(sale.revenue || 0).toLocaleString()} FCFA
										</p>
									</div>
								</div>
							))
							...
							revenueAnalytics.monthlyRevenue.slice(0, 6).map((rev, index) => (
								...
								>
									...
									<p className="text-base font-bold text-green-600">
										{(rev.revenue || 0).toLocaleString()} FCFA
									</p>
								</div>
							))
						:	<div className="text-center py-8 text-gray-500">
						...
						{stats?.topProducts && stats.topProducts.length > 0 ?
							<div className="space-y-3">
								{stats.topProducts.map((dish, index) => (
									<div
										key={dish.id || index}
										className="flex items-center justify-between p-3 bg-harvests-light rounded-lg hover:bg-gray-100 transition-colors"
									>
										<div className="flex items-center flex-1">
											...
											<div className="ml-3 flex-1 min-w-0">
												<p className="text-sm font-medium text-gray-900 truncate">
													{toPlainText(dish.name, "Plat")}
												</p>
												<p className="text-xs text-gray-500">
													{toPlainText(dish.category, "plat")}
												</p>
											</div>
										</div>
