...
import { ProductImageGallery, ProductActions, VendorCard } from '../components/product';
...
import { formatPrice, getCategoryLabel, getStatusConfig } from '../utils/productUtils';
import { 
  FiArrowLeft, FiStar, FiPackage, FiHeart, FiShoppingCart, FiShield, 
...
const ProductDetail = () => {
  ...
  const {
    ...
    isFavorite, setIsFavorite, favoritesCount, setFavoritesCount, loadReviews
  ...
  const handleAddToCart = () => {
    if (product) {
      ...
      showSuccess(`${quantity} article${quantity > 1 ? 's' : ''} ajouté${quantity > 1 ? 's' : ''} au panier`, 'Produit ajouté');
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: toPlainText(product?.name), text: toPlainText(product?.description), url: window.location.href });
    } else {
  ...
  const handleVoteHelpful = async (reviewId) => {
    try { await reviewService.voteHelpful(reviewId); loadReviews(); } catch (e) { console.error(e); }
  };

  const handleVoteUnhelpful = async (reviewId) => {
    try { await reviewService.voteUnhelpful(reviewId); loadReviews(); } catch (e) { console.error(e); }
  };

  const handleSubmitReview = async (reviewData) => {
    try {
      await reviewService.createReview({ ...reviewData, productId: product._id, producer: producer?._id });
      ...
      else if (msg.includes('pas encore complétée')) msg = 'Votre commande n\'est pas encore complétée.';
      else if (msg.includes('déjà laissé un avis')) msg = 'Vous avez déjà laissé un avis pour cette commande';
      showError(msg);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-harvests-light flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">Le produit n'existe pas ou n'est plus disponible</p>
          <button onClick={() => navigate('/products')} className="inline-flex items-center px-4 py-2 bg-harvests-green text-white rounded-md">
            <FiArrowLeft className="h-4 w-4 mr-2" />Retour
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-harvests-light flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">Ce produit n'est pas encore approuvé.</p>
          <button onClick={() => navigate('/products')} className="inline-flex items-center px-4 py-2 bg-harvests-green text-white rounded-md">
            <FiArrowLeft className="h-4 w-4 mr-2" />Retour aux produits
          </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-harvests-light">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="space-y-6 relative">
            ...
            <div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {product.status !== 'approved' && (
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                      <FiShield className="w-3 h-3 mr-1" />Non public
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                <StarRating rating={reviewStats?.averageRating || 0} size="md" showText />
                {reviewStats?.totalReviews > 0 && <span className="text-sm text-gray-600">({reviewStats.totalReviews} avis)</span>}
                <div className="flex items-center text-sm text-gray-600"><FiPackage className="h-4 w-4 mr-1" />{product.inventory?.quantity || 0} En stock</div>
                {product.stats && (
                  <>
                    <div className="flex items-center text-sm text-gray-600"><FiEye className="h-4 w-4 mr-1" />{product.stats.views || 0} vues</div>
                    <div className="flex items-center text-sm text-gray-600"><FiHeart className="h-4 w-4 mr-1" />{favoritesCount} favoris</div>
                  </>
                )}
              </div>
            </div>
            ...
            {showAddedToCart && (
              <div className="absolute top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center">
                <FiShoppingCart className="h-4 w-4 mr-2" />{quantity} article{quantity > 1 ? 's' : ''} ajouté{quantity > 1 ? 's' : ''} !
              </div>
            )}
        ...
        <div className="mt-12">
              ...
              {['description', 'specifications', 'reviews'].map(tab => (
                <button
                  ...
                  className={`py-2 px-1 border-b-2 font-medium text-sm ${
                    activeTab === tab ? 'border-harvests-green text-harvests-green' : 'border-transparent text-gray-500 hover:text-gray-700'
                  }`}
                >
                  {tab === 'description' ? 'Description' : tab === 'specifications' ? 'Spécifications' : `Avis (${reviews.length})`}
                </button>
              ))}
            </nav>
          </div>
          ...
          <div className="py-6">
            {activeTab === 'description' && <p className="text-gray-600 leading-relaxed">{productDescription}</p>}
            {activeTab === 'specifications' && <ProductSpecifications product={product} producer={producer} user={user} statusConfig={statusConfig} />}
            {activeTab === 'reviews' && (
              <div className="space-y-6">
                {canLeaveReview && !showReviewForm && (
                  <div className="bg-primary-50 border border-primary-200 rounded-lg p-4 flex items-center justify-between">
                    <div>
                      <h3 className="text-lg font-medium text-primary-500">Avez-vous acheté ce produit ?</h3>
                      <p className="text-primary-700 text-sm mt-1">Partagez votre expérience</p>
                    </div>
                    <button onClick={() => setShowReviewForm(true)} className="inline-flex items-center px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700">
                      <FiStar className="h-4 w-4 mr-2" />Laisser un avis
                    </button>
                  </div>
                )}
                {showReviewForm && (
                  <div className="bg-white border border-gray-200 rounded-lg p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button onClick={() => setShowReviewForm(false)} className="text-gray-400 hover:text-gray-600"><FiX className="h-5 w-5" /></button>
                    </div>
                    <SimpleReviewForm product={product} producer={producer} onSubmit={handleSubmitReview} onCancel={() => setShowReviewForm(false)} />
                  </div>
                )}
                <ReviewList reviews={reviews} stats={reviewStats} loading={reviewsLoading} onVoteHelpful={handleVoteHelpful} onVoteUnhelpful={handleVoteUnhelpful} currentUserId={user?._id} showProductInfo={false} />
              </div>
            )}
          </div>
        </div>
