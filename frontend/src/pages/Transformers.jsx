...
const Transformers = () => {
  ...
  const filteredTransformers = transformers.filter((t) => {
    ...
    const name = (t.shopInfo?.shopName || t.companyName || `${t.firstName} ${t.lastName}`).toLowerCase();
          ...
          viewMode === 'grid' ? (
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
              {filteredTransformers.map((transformer) => {
                const name = transformer.shopInfo?.shopName || transformer.companyName || `${transformer.firstName} ${transformer.lastName}`;
                    ...
                    <div className="px-4 -mt-7 flex items-end justify-between relative z-10">
                      <div className="w-14 h-14 rounded-xl bg-white p-0.5 border border-gray-200/90 shadow-md overflow-hidden flex-shrink-0">
                        {transformer.shopInfo?.shopLogo ? (
                          <img src={transformer.shopInfo.shopLogo} alt={name} className="w-full h-full object-cover rounded-lg" />
                        ) : (
