...
export const orderService = {
	...
	estimateOrder: (estimateData) =>
		apiRequest.post("/orders/estimate", estimateData),
	...
	cancelOrder: (id, reason) =>
		apiRequest.patch(`/orders/${id}/cancel`, { reason }),
	...
	shipOrder: (id, trackingData) =>
		apiRequest.patch(`/orders/${id}/ship`, trackingData),

	// Marquer comme livrée
	deliverOrder: (id, deliveryData) =>
		apiRequest.patch(`/orders/${id}/deliver`, deliveryData),
	...
	getUserOrders: (userId, params = {}) =>
		apiRequest.get(`/orders/user/${userId}`, { params }),

	// Obtenir les commandes d'un producteur
	getProducerOrders: (producerId, params = {}) =>
		apiRequest.get(`/orders/producer/${producerId}`, { params }),

	// Obtenir les commandes par statut
	getOrdersByStatus: (status, params = {}) =>
		apiRequest.get(`/orders/status/${status}`, { params }),
	...
	getPendingOrders: (params = {}) =>
		apiRequest.get("/orders/pending", { params }),

	// Obtenir les commandes expédiées
	getShippedOrders: (params = {}) =>
		apiRequest.get("/orders/shipped", { params }),

	// Obtenir les commandes livrées
	getDeliveredOrders: (params = {}) =>
		apiRequest.get("/orders/delivered", { params }),

	// Obtenir les commandes annulées
	getCancelledOrders: (params = {}) =>
		apiRequest.get("/orders/cancelled", { params }),

	// Rechercher des commandes
	searchOrders: (query, params = {}) =>
		apiRequest.get("/orders/search", {
			params: { ...params, q: query },
		}),

	// Obtenir les commandes par période
	getOrdersByPeriod: (startDate, endDate, params = {}) =>
		apiRequest.get("/orders/period", {
			params: { ...params, startDate, endDate },
		}),

	// Générer une facture
	generateInvoice: (orderId, currency, exchangeRate) =>
		apiRequest.get(`/orders/${orderId}/invoice`, {
	...
	createReturn: (orderId, returnData) =>
		apiRequest.post(`/orders/${orderId}/return`, returnData),

	// Traiter un retour
	processReturn: (returnId, action, data = {}) =>
		apiRequest.patch(`/orders/returns/${returnId}/process`, {
			action,
			...data,
		}),
};
