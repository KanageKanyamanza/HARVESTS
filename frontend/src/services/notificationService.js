...
class NotificationService {
	...
	async getNotifications(page = 1, limit = 20) {
		try {
			...
			const notifications = (response.data.data.notifications || []).map(
				(notif) => {
					return {
						id: notif._id,
						title: notif.title,
			...
			if (error.response?.status === 403 || error.response?.status === 404) {
				console.warn(
					"[NotificationService] Endpoint notifications non disponible - utilisation du mode local"
				);
			...
			if (error.response?.status === 401) {
				console.warn(
					"[NotificationService] Non authentifié - notifications non disponibles"
				);
				return { notifications: [], unreadCount: 0 };
			}

			console.error(
				"[NotificationService] Erreur lors de la récupération des notifications:",
				error
			);
			throw error;
		}
	...
	async markAsRead(notificationId) {
		try {
			...
			if (error.response?.status === 403 || error.response?.status === 404) {
				console.log(
					"Endpoint markAsRead non disponible - action locale uniquement"
				);
				return { success: true };
			}
	...
	async markAllAsRead() {
		try {
			...
			if (error.response?.status === 403 || error.response?.status === 404) {
				console.log(
					"Endpoint markAllAsRead non disponible - action locale uniquement"
				);
				return { success: true };
			}
	...
	async deleteNotification(notificationId) {
		try {
			...
			if (error.response?.status === 403 || error.response?.status === 404) {
				console.log(
					"Endpoint deleteNotification non disponible - action locale uniquement"
				);
				return { success: true };
			}
	...
	async deleteAllNotifications() {
		try {
			...
			console.log(
				"Suppression de toutes les notifications - non implémentée côté backend"
			);
			return { success: true };
		} catch (error) {
			console.error(
	...
	createOrderNotification(order) {
		return this.createLocalNotification(
			...
			} d'un montant de ${order.total?.toLocaleString("fr-FR")} FCFA`,
			{
				orderId: order._id,
