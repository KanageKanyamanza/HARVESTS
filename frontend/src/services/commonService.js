...
const commonService = {
	...
	getBankAccount: () =>
		api
			.get("/users/me/financial-info")
			.then((res) => ({ ...res, data: res.data.data.bankAccount })),
	updateBankAccount: (data) =>
		api.patch("/users/me/financial-info", { bankAccount: data }),
	...
	getPaymentMethods: () =>
		api
			.get("/users/me/financial-info")
			.then((res) => ({ ...res, data: res.data.data.paymentMethods })),
	updatePaymentMethods: (data) =>
		api.patch("/users/me/financial-info", { paymentMethods: data }),
	...
	getNotificationPreferences: () => api.get("/notifications/my/preferences"),
	updateNotificationPreferences: (data) =>
		api.patch("/notifications/my/preferences", data),

	// Informations de vérification
	getVerificationStatus: () => api.get("/users/me/verification-status"),
	uploadVerificationDocument: (formData) =>
		api.post("/users/me/verification-documents", formData),
	updateVerificationDocument: (docId, data) =>
		api.patch(`/users/me/verification-documents/${docId}`, data),
	...
	updateDeliveryAddress: (addressId, data) =>
		api.patch(`/users/me/delivery-addresses/${addressId}`, data),
	deleteDeliveryAddress: (addressId) =>
		api.delete(`/users/me/delivery-addresses/${addressId}`),
	setDefaultDeliveryAddress: (addressId) =>
		api.patch(`/users/me/delivery-addresses/${addressId}/set-default`),
	...
	getMonthlyStats: (year, month) =>
		api.get(`/users/me/monthly-stats/${year}/${month}`),

	// Avis et évaluations
	getMyReviews: (page = 1, limit = 10) =>
		api.get(`/users/me/reviews?page=${page}&limit=${limit}`),
	...
	updateShopDescription: (data) =>
		api.patch("/users/me/shop-description", data),

	// Paramètres de compte
	getAccountSettings: () => api.get("/users/me/account-settings"),
	updateAccountSettings: (data) =>
		api.patch("/users/me/account-settings", data),
