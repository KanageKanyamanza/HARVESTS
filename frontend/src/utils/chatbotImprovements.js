...
export const extractKeywords = (message) => {
  ...
  const stopWords = new Set([
    ...
    'acheter', 'achete', 'pour', 'du', 'de', 'la', 'le', 'les', 'des', 'un', 'une',
    'dans', 'sur', 'avec', 'sans', 'par', 'pour', 'vers', 'chez', 'à', 'au', 'aux',
    'comment', 'combien', 'où', 'quand', 'quoi', 'qui', 'pourquoi'
  ]);
  ...
  return [...new Set(expandedWords)]; // Retirer les doublons
};
  ...
  for (const intent of intents) {
    ...
    if (score > 0) {
      const matchedKeywords = keywords.filter(k => 
        normalized.includes(k.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''))
      );
...
export const understandQuestion = (message) => {
  ...
  const questionTypes = {
    'how': {
      pattern: /comment|comment faire|comment puis|comment peux|comment peut|comment procéder|comment faire pour/,
    ...
    'when': {
      pattern: /quand|à quelle heure|combien de temps|délai|délais|temps|quand est|quand sera/,
      examples: ['quand', 'à quelle heure', 'combien de temps']
    },
    'where': {
      pattern: /où|où est|où sont|où puis|où peux|où trouver|où acheter|où se trouve/,
    ...
    'how_much': {
      pattern: /combien|prix|coût|coûte|coute|tarif|montant|cher|gratuit|payant/,
      examples: ['combien', 'prix', 'coût']
    },
    'yes_no': {
      pattern: /est-ce que|est ce que|peut-on|peut on|avez-vous|avez vous|y a-t-il|y a t il|existe|disponible/,
...
export const handleSearchError = (error, searchTerm) => {
  ...
  if (error.response?.status >= 500) {
    return {
      message: 'Désolé, un problème technique est survenu. Veuillez réessayer dans quelques instants.',
      suggestions: [
