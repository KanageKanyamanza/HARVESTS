...
export const getVendorName = (vendor) => {
  if (!vendor) return '';
  return vendor.farmName || vendor.companyName || vendor.restaurantName || vendor.businessName || 
    `${vendor.firstName || ''} ${vendor.lastName || ''}`.trim() || 'Vendeur';
};
...
export const getStatusConfig = (status) => {
  const configs = {
    'approved': { color: 'bg-green-100 text-green-800', text: 'Disponible', iconName: 'check' },
    'pending-review': { color: 'bg-yellow-100 text-yellow-800', text: 'En cours de révision', iconName: 'clock' },
    'draft': { color: 'bg-blue-100 text-blue-800', text: 'Brouillon', iconName: 'package' },
    'rejected': { color: 'bg-red-100 text-red-800', text: 'Rejeté', iconName: 'x' },
    'inactive': { color: 'bg-gray-100 text-gray-800', text: 'Indisponible', iconName: 'x' }
  };
...
export const parseProductName = (name) => {
  ...
  if (typeof name === 'object') {
    return name.fr || name.en || name.value || Object.values(name)[0] || 'Produit sans nom';
  }
  return String(name);
};
