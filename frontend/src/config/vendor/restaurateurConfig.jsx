...
import { VendorReviewsList, VendorHours, VendorEmptyState } from '../../components/common/vendor';
...
export const restaurateurConfig = {
  ...
  getVendorSubtitle: (r) => RESTAURANT_TYPES[r.restaurantType] || r.restaurantType,
  ...
  getVendorStats: (restaurateur, dishes, reviews = []) => {
    ...
    return [
      ...
      { icon: <FiPackage className="w-5 h-5 text-orange-500" />, value: dishes.length, label: 'Plats' },
      { icon: <FiUsers className="w-5 h-5 text-green-500" />, value: reviewCount, label: 'Avis' },
      { icon: <FiCalendar className="w-5 h-5 text-purple-500" />, value: new Date(restaurateur.createdAt).getFullYear(), label: 'Membre depuis' }
    ];
  },
  ...
  getTabLabel: (tab) => ({ 'menu': 'Menu', 'reviews': 'Avis', 'hours': 'Horaires' }[tab] || tab),
              ...
              >
                <div className="h-48 bg-gray-200 relative">
                  {helpers.getItemImage(item) ? (
                    <img src={helpers.getItemImage(item)} alt={helpers.getItemName(item)} className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="p-4">
                  <h3 className="font-semibold text-gray-900 mb-2">{helpers.getItemName(item)}</h3>
                  <p className="text-gray-600 text-sm mb-3 line-clamp-2">{helpers.getItemDescription(item)}</p>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-lg font-bold text-orange-600">{helpers.formatPrice(helpers.getItemPrice(item))}</span>
                    <span className="text-sm text-gray-500">{helpers.getItemExtraInfo(item)}</span>
                  </div>
                  {item.trackQuantity && (
                    <div className="mb-2">
                  ...
                  <button 
                    ...
                    className={`w-full mt-3 py-2 px-4 rounded-lg transition-colors flex items-center justify-center ${
                      outOfStock ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : `${helpers.getItemButtonColor} text-white`
                    }`}
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  >
                    ...
                    {outOfStock ? 'Rupture de stock' : helpers.getItemButtonText}
                  </button>
                </div>
              </div>
            );
