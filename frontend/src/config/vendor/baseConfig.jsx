...
export const formatPrice = (price) => new Intl.NumberFormat('fr-FR', {
  style: 'currency',
  currency: 'XOF',
  minimumFractionDigits: 0
}).format(price);

export const formatPriceOrQuote = (price) => price ? formatPrice(price) : 'Sur devis';
...
export const getBaseContact = (vendor) => {
  ...
  if (vendor.address || vendor.city) {
    contact.push({
      ...
      text: `${vendor.address ? vendor.address + ', ' : ''}${vendor.city || ''}${vendor.region ? ', ' + vendor.region : ''}`
    });
  }
  return contact;
};
